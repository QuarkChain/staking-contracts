...
const { utils, rlp } = require("ethereumjs-util");
...
exports.rewradsTree = async function rewradsTree(validators, accountState) {
  let leafs = [];
  let i = 0;
  validators.map((key) => {
    leafs[i++] = utils.keccak256(web3.eth.abi.encodeParameters(["uint256", "uint256"], [key, accountState[key]]));
  });
  return new MerkleTree(leafs);
};

exports.buildTreeFee = async function buildTreeFee(validators, accountState, checkpointIndex) {
  let leafs = [];
  let i = 0;
  validators.map((key) => {
    const state = accountState[key][checkpointIndex] || [0];
    leafs[i++] = utils.keccak256(web3.eth.abi.encodeParameters(["address", "uint256"], [key, state[0].toString()]));
  });
  return new MerkleTree(leafs);
};
...
// build
exports.getTxProof = async function getTxProof(tx, block) {
  const db = new Level("./testdb");
  const txTrie = new BaseTrie(db);
  ...
  const proof = await BaseTrie.createProof(txTrie, rlp.encode(tx.transactionIndex));
  return proof;
};

exports.verifyTxProof = function verifyTxProof(proof) {
  ...
  try {
    ...
    for (var i = 0; i < len; i++) {
      ...
      const encodedNode = Buffer.from(utils.keccak256(rlp.encode(currentNode)), "hex");
...
exports.getReceiptBytes = function getReceiptBytes(receipt) {
  return rlp.encode([
    utils.toBuffer(
...
exports.getReceiptProof = async function getReceiptProof(receipt, block, web3, receipts) {
...
exports.verifyReceiptProof = function verifyReceiptProof(proof) {
