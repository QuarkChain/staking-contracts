...
async function main() {
  ...
  let globalConfig = new BridgeConfig()
  let deployParams = globalConfig.getLightClientParams()
  
  let stakingFac = await ethers.getContractFactory("StakingMock")
  let staking = await stakingFac.deploy()
  staking = await staking.deployed();
  let staking_addr = staking.address
  let setvTx = await staking.setValidators(deployParams.validators)
  await setvTx.wait()
  ...
  console.log("info:",validators,powers)
